...
class LinesSatPage extends StatefulWidget {
  final Future<FirebaseApp> _initialization = Firebase.initializeApp();
  @override
  _LinesSatPageState createState() => _LinesSatPageState();
}

class _LinesSatPageState extends State<LinesSatPage> {

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Soboty', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold),),
        centerTitle: true,
        backgroundColor: Colors.transparent,
        elevation: 0.0,
      ),
      body: StreamBuilder(
          stream: FirebaseFirestore.instance.collection('Soboty').orderBy('l').snapshots(),
            ...
            ButtonBar(
              children: [
                FlatButton(
                  onPressed: (){
                    linia = widget.x;
                    Navigator.pushNamed(context, '/stopsSat', arguments: {'linia': linia, 'kierunek': 1});
                  },
                ...
                FlatButton(
                  onPressed: (){
                    linia = widget.x;
                    Navigator.pushNamed(context, '/stopsSat', arguments: {'linia': linia, 'kierunek': 2});
                  },
